...
from models import LlamaForCausalLM_qat_mix
...
Weight_bit = "W4"
Activation_bit = "A8"
Activation_low_bit = "A4"
Mix_low_bit_percent = 0.5
####################################################
...
MODEL_NAME = f'Llama-58M-qat-mix-lowbit{Mix_low_bit_percent}{Activation_low_bit}-{Weight_bit}{Activation_bit}-epoch{NUM_EPOCHS}-warmup{WARMUP_STEPS}-lr{LR}-bs{BATCH_SIZE}-temp{TEMPERATURE}-alpha{ALPHA}'
...
# student = LlamaForCausalLM_qat_mix(config)
student = LlamaForCausalLM_qat_mix.from_pretrained(student_dir)
