...
from models import LlamaForCausalLM_pact
...
MODEL_NAME = f'Llama-58M-pact-{Weight_bit}{Activation_bit}-epoch{NUM_EPOCHS}-warmup{WARMUP_STEPS}-lr{LR}-bs{BATCH_SIZE}-temp{TEMPERATURE}-alpha{ALPHA}'
...
# student = LlamaForCausalLM_pact(config)
student = LlamaForCausalLM_pact.from_pretrained(student_dir)
