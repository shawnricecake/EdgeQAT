...
from models import LlamaForCausalLM_qat_distribution, LlamaForCausalLM_fp16


# Training Recipe   # xuan: todo
####################################################
NUM_EPOCHS = 6      # normal pretrain: 6
WARMUP_STEPS = 200  # normal pretrain: 200
LR = 2.5e-4         # normal pretrain: 2.5e-4
...
Weight_bit = "W4"
Activation_bit = "A8"
Quantization_type = "modeLayer"     # or "modeKernel"
####################################################

# All Paths
########################################################################################################
student_dir = "/path/llama-58m"
teacher_dir0 = "/path/llama-58m"
...
MODEL_NAME = f'Llama-58M-qat-distribution-guide-{Weight_bit}{Activation_bit}-{Quantization_type}-epoch{NUM_EPOCHS}-warmup{WARMUP_STEPS}-lr{LR}-bs{BATCH_SIZE}-temp{TEMPERATURE}-alpha{ALPHA}'
...
# student = LlamaForCausalLM_qat_distribution(config)
student = LlamaForCausalLM_qat_distribution.from_pretrained(student_dir)

teacher0 = LlamaForCausalLM_fp16.from_pretrained(teacher_dir0)
...
class DistillationTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        student_q = model.model.q_all_layer
        student_k = model.model.k_all_layer
        student_attn = model.model.attn_all_layer
        ...
        with torch.no_grad():
            ...
            for i_th_teacher, teacher in enumerate(self.teachers):
                ...
                if i_th_teacher == 0:
                    teacher_attn = teacher.model.attn_all_layer
