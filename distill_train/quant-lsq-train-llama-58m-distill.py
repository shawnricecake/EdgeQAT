...
from models import LlamaForCausalLM_lsq
...
MODEL_NAME = f'Llama-58M-lsq-{Weight_bit}{Activation_bit}-{PER_CHANNEL}-epoch{NUM_EPOCHS}-warmup{WARMUP_STEPS}-lr{LR}-bs{BATCH_SIZE}-temp{TEMPERATURE}-alpha{ALPHA}'
...
# student = LlamaForCausalLM_lsq(config)
student = LlamaForCausalLM_lsq.from_pretrained(student_dir)
